"""
Baseline pipeline for web-only users (ck_user_events_with_url.csv).
Trains WITHOUT page context — establishes the baseline to compare against
the page-context variant later.

Isolated from the main pipeline:
  - cache dir  : cache_web_no_ctx/
  - model file : cache_web_no_ctx/two_tower_web_no_ctx.pt
  - Milvus     : video_embeddings_web_no_ctx / user_embeddings_web_no_ctx
"""
import ast
import logging
import pickle
...
CACHE_DIR        = Path("cache_web_no_ctx")
CSV_FILE         = "ck_user_events_with_url.csv"
VIDEO_CSV_FILE   = "new_video_data.csv"
MODEL_FILE       = CACHE_DIR / "two_tower_web_no_ctx.pt"
VIDEO_COLLECTION = "video_embeddings_web_no_ctx"
USER_COLLECTION  = "user_embeddings_web_no_ctx"
MIN_INTERACTIONS = 10
TEST_RATIO       = 0.2
...
def load_events() -> pd.DataFrame:
    ...
    # Drop URL — not used in baseline (no page context)
...
def evaluate(
    ...
    print("\n" + "=" * 60)
    print("EVALUATION RESULTS — web-only baseline (no page context)")
    print("=" * 60)
    ...
    # Save metrics for later comparison with page-context run
    metrics = {
        "variant": "web_no_ctx",
        "n_eval": n_eval,
    ...
    logger.info("Saved eval metrics → cache_web_no_ctx/eval_metrics.pkl")
...
def main():
    ...
    logger.info("Training two-tower model (web-only, no page context)...")
    model = TwoTowerModel(
